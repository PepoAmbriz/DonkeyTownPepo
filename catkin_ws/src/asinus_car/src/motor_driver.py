...
class motor_driver_node: 
	def __init__(self,car_id,i2c_bus=1,i2c_addr=0x08): 
		self.car_id = car_id
		self.motors = motors(i2c_bus,i2c_addr)
		self.motors.stop()
	...
	def on_drive(self,speed_msg): 
		...
		self.motor_st.speed.leftMotor = sp_left[0]
		self.motor_st.speed.rightMotor = sp_right[0]
		self.motor_st.voltage = volt[0] 
		sleep(0.01) #Increase time gap between i2c transactions.
		try:
